...
<form class="box_form" action="" method="post">
    ...
    if(isset($_POST['name_key'])){
        ...
        if($name_key==''){
            ...
            if($id_edit==''){
                $query_add=mysql_query("INSERT INTO `key_lang` (`name_key`) VALUES ('$name_key');");
                if($query_add){
                    $name_key='';
                ...
                $query_add=mysql_query("UPDATE `key_lang` SET `name_key` = '$name_key' WHERE `id` = '$id_edit';");
                if($query_add){
...
if(isset($_GET['delete'])){
    ...
    $query_delete=mysql_query("DELETE FROM `key_lang` WHERE ((`id` = '$id_delete'));");
