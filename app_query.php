...
if($func=="download_lang"){
    $id=$_POST['id'];
    echo json_encode($app_contacts->list_lang[$id]);
    exit;
}
...
if($func=="get_list_contacts"){
    ...
    $arr_contacts=array();
    if($search==''){
        $query_list_contact=mysql_query("SELECT * FROM  carrotsy_virtuallover.app_my_girl_user_$lang WHERE `address` != '' AND `sdt` != '' AND `status`='0' ORDER BY RAND() LIMIT 20");
    }else{
        $query_list_contact=mysql_query("SELECT * FROM  carrotsy_virtuallover.app_my_girl_user_$lang WHERE ((MATCH (`sdt`) AGAINST ('$search')) OR `name` LIKE '%$search%')  AND `sdt` != '' AND `status` = '0' LIMIT 60");
    }
...
if($func=='view_contact'){
    ...
    if(mysql_num_rows($query_contact)>0){
        ...
        if($contacts['sex']=='1'){
            $value_item->icon=$url.'/image/girl.png';
        }else{
            $value_item->icon=$url.'/image/boy.png';
        }
        ...
        $value_item=new Item_value();
        if($contacts['sex']=='1'){
            $value_item->icon=$url.'/image/call_girl.png';
        }else{
            $value_item->icon=$url.'/image/call_boy.png';
        }
        ...
        //Nhắn tin - sms
        $value_item=new Item_value();
        $value_item->icon=$url.'/image/sms.png';
        $value_item->key='sms';
        ...
        if($contacts['address']!=''){
            $value_item=new Item_value();
            $value_item->icon=$url.'/image/maps.png';
        ...
        $value_item->key='sex';
        if($contacts['sex']=='0'){
            $value_item->icon=$url.'/image/sex_boy.png';
            $value_item->value='sex_boy';
        }else{
            $value_item->icon=$url.'/image/sex_girl.png';
        ...
        if(mysql_num_rows($query_check_field)>0){
            ...
            foreach($data_field as $i_key=>$i_val){
                ...
                if(mysql_num_rows($query_get_field)>0){
                    ...
                    $value_item=new Item_value();
                    $value_item->icon=$url.'/field_icon/'.$data_item_field['id'].'.png';
        ...
        if(mysql_num_rows($query_app_sheep)>0){
            ...
            $value_item=new Item_value();
            $value_item->icon=$url.'/image/sheep.png';
        ...
        $value_item=new Item_value();
        $value_item->icon=$url.'/image/status.png';
...
if($func=='view_contact_edit'){
    ...
    //Tên
    $value_item=new Item_value();
    $value_item->icon=$url.'/image/name.png';
    $value_item->key='account_name';
    $value_item->value=$contacts['name'];
    ...
    //số điện thoại
    $value_item=new Item_value();
    $value_item->icon=$url.'/image/contact_phone.png';
    $value_item->key='phone';
    $value_item->value=$contacts['sdt'];
    ...
    //Giới tính
    $value_item=new Item_value();
    $value_item->icon=$url.'/image/sex.png';
    $value_item->key='sex';
...
if($func=='get_gprs'){
    ...
    $place="http://maps.googleapis.com/maps/api/geocode/json?latlng=$location_lat,$location_lon&sensor=true&key=AIzaSyCcYpVI8I4osXUeqWkPe-nPrakxNnaND5I";
...
if($func=='get_list_field'){
    ...
    while($row_field=mysql_fetch_array($query_get_list_field)){
        ...
        $item_field->icon=$url.'/field_icon/'.$item_field->id.'.png';
        if($row_field['tip']!=''){
...
if($func=='get_list_company'){
    ...
    While($row_contacts=mysql_fetch_array($query_list_company)){
        ...
        if(file_exists('company_img/'.$lang.'/'.$contacts_item->id.'.png')){
            $contacts_item->avatar=$url.'/thumb.php?src='.$url.'/company_img/'.$lang.'/'.$contacts_item->id.'.png&size=80x80';
        }else{
            $contacts_item->avatar=$url.'/thumb.php?src='.$url.'/image/pic_contact.png&size=80x80';;
        }
        array_push($arr_contacts,$contacts_item);
    }
...
if($func=='view_contact_company'){
    ...
    if(file_exists('company_img/'.$lang.'/'.$contact_company->id.'.png')){
        $contact_company->avatar=$url.'/thumb.php?src='.$url.'/company_img/'.$lang.'/'.$contact_company->id.'.png&size=80x80';
    }else{
        $contact_company->avatar=$url.'/thumb.php?src='.$url.'/image/pic_contact.png&size=80x80';;
    }
    ...
    //Tên
    $value_item=new Item_value();
    $value_item->icon=$url.'/image/name_company.png';
    $value_item->key='account_name';
...
if($func=='view_company_edit'){
    ...
    if(file_exists('company_img/'.$lang.'/'.$id_company.'.png')){
        $value_item->icon=$url.'/thumb.php?src='.$url.'/company_img/'.$lang.'/'.$id_company.'.png&size=80x80';
        $value_item->value=$url.'/thumb.php?src='.$url.'/company_img/'.$lang.'/'.$id_company.'.png&size=80x80';
    }else{
        $value_item->icon=$url.'/thumb.php?src='.$url.'/image/pic_contact.png&size=80x80';
        $value_item->value=$url.'/thumb.php?src='.$url.'/image/pic_contact.png&size=80x80';
    }
    $value_item->key='account_avatar';
    $value_item->tip='account_avatar_tip';
    $value_item->type='3';
