...
$url_cur=$url.'/?view=value_lang';

if(isset($_GET['lang'])){
    $lang_key=$_GET['lang'];
    $query_get_val=mysql_query("SELECT * FROM `lang_value` WHERE `id_country` = '$lang_key' LIMIT 1");
...
if(isset($_POST['lang_key'])){
    ...
    $check_value_lang=mysql_query("SELECT * FROM `lang_value` WHERE `id_country` = '$lang_key' LIMIT 1");
    if(mysql_num_rows($check_value_lang)==0){
        $query_add_lang=mysql_query("INSERT INTO `lang_value` (`id_country`, `value`) VALUES ('$lang_key', '$data_val_inst');");
        if($query_add_lang){
        ...
        $query_update_lang=mysql_query("UPDATE `lang_value` SET `value` = '$data_val_inst' WHERE `id_country` = '$lang_key' ");
...
<tr>
    ...
    <td ><input style="width: 200px;" type="text" name="<?php echo $row_key['name_key']; ?>" value="<?php echo $data_val->{$row_key['name_key']};?>" /></td>
    <td>
...
function change_lang_to(emp){
    ...
    window.location="<?php echo $url_cur;?>&lang=<?php echo $lang_key;?>&lang_to="+val_lang;
}
